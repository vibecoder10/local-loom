// Build a 16:9 landscape talking-head project's index.html from scene-plan.json + the take package's words.
// Counterpart to build-short.mjs (portrait). Usage: node build-talking-head-16x9.mjs <project-dir>
...
const fail = (m) => { console.error('build-talking-head-16x9: ' + m); process.exit(1); };
for (const f of ['scene-plan.json', 'take/manifest.json', 'take/words.remapped.json']) if (!fs.existsSync(path.join(project, f))) fail(`missing ${f} (run scaffold-talking-head-16x9.mjs first)`);
...
// ---- layout (1920x1080): panel on the left, camera plate on the right, captions under both ----
const panel = { x: 64, y: 64, w: 1090, h: 760 };
const plate = { x: panel.x + panel.w + 40, y: 64, w: 1920 - 64 - (panel.x + panel.w + 40), h: 760 };
const capY = panel.y + panel.h + 40;
const layout = {
  panel, plate,
  capsPlate: { x: 64, y: capY, w: 1792, h: Math.max(120, 1080 - 64 - capY) },
  capsFull: { x: 160, y: 850, w: 1600, h: 170 },
};
if (capY + 110 > 1080 - 24) fail(`panel too tall (${panel.h}px): captions would not fit under it`);

// ---- caption groups from the remapped words (landscape: a bit more room than portrait, less than the tutorial screen) ----
...
const template = fs.readFileSync(path.join(skillRoot, 'templates/talking-head-16x9/template.html'), 'utf8');
const html = template
...
console.log(`Built ${path.join(project, 'index.html')}: ${duration}s, ${plan.scenes.length} scene(s), ${groups.length} caption group(s), ${iconSlugs.size} brand icon(s). Panel ${panel.w}x${panel.h}, plate ${plate.w}x${plate.h}.`);
